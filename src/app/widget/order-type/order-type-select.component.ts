...
import {OrderTypeSelectBaseComponent} from './base/order-type-select.component.base';
import {SELECT_CONFIG} from './config/select-config.js';

@Component({
	selector: 'app-order-type-select',
	templateUrl: './html/order-type-select.html',
	providers: [
		{
			provide: NG_VALUE_ACCESSOR,
			useExisting: OrderTypeSelectComponent,
			multi: true
		}
	]
})
export class OrderTypeSelectComponent extends OrderTypeSelectBaseComponent {
	constructor(
		http: HttpClient,
		router: Router,
